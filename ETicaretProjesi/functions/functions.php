<?php
    ...
    // Sepete Ekleme işlemi yapılıyor.
    function add_cart() {
        global $db;
        if(isset($_GET['add_cart'])) {
            ...
            if(mysqli_num_rows($run_check) > 0) {
                echo "<script>alert('Ürün ekleme işlemi başarılı')</script>";
    ...
    // sepetteki ürün sayısı veriliyor.
    function items() {
        global $db;
        $ip_add = getRealUserIP();
    ...
    function getProCats() {
        global $db;
        $get_pro_cats = "select * from product_categories";
        $run_pro_cats = mysqli_query($db, $get_pro_cats);
        while($row_pro_cats = mysqli_fetch_array($run_pro_cats)) {
            $pro_cat_id = $row_pro_cats['p_cat_id'];
            $pro_cat_title = $row_pro_cats['p_cat_title'];
            echo "<li><a href='shop.php?p_cat=$pro_cat_id'>$pro_cat_title</a></li>";
        }
    }
    ...
    function getPCatPro() {
        ...
        if(isset($_GET['p_cat'])) {
            ...
            if($count == 0) {
                echo "
                    <div class='box'>
                        <h1>Bu Kategoriye ait ürün BULUNAMADI !!!</h1>
                    </div>
                ";
            } else {
                echo "
